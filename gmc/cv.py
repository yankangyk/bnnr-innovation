...
def getKfoldCrossValidMatIndSet(Wdr, nfold, CVtype, NegType=None, seed=None):
    ...
    Supports three CV strategies (Wdr is (n_dis, n_drug): diseases are ROWS,
    drugs are COLUMNS):
      - CVa: random split by association pairs
      - CVr: hide all associations for some drugs (zero their columns)
      - CVc: hide all associations for some diseases (zero their rows)
    ...
    if CVtype == 'CVa':
        ...
        # CVr: hold out all associations for some DRUGS (zero their columns).
        ...
        # CVc: hold out all associations for some DISEASES (zero their rows).
