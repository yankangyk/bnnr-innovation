...
\caption{Main results (10-fold CVc). AUPR for BNNR, GRMC with dense graphs ($\alpha=0.7$), and SGRMC (density-adaptive KNN, $\alpha=0.7$) on Fdataset, Cdataset, and DNdataset. Error bars show $\pm 1$ std.}
\label{fig:main}
\end{figure*}

SGRMC substantially outperforms BNNR on all three datasets (Figure~\ref{fig:main}). The AUPR improvements are $+20.6\%$ (Fdataset), $+37.9\%$ (Cdataset), and $+26.6\%$ (DNdataset). AUROC improvements are most pronounced on the ultra-sparse DNdataset ($0.9493$ vs.\ $0.8391$, $+13.1\%$ relative gain), reflecting the graph Laplacian filter's ability to suppress false positives across the full ROC curve.
