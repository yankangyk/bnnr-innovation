...
\begin{enumerate}
    ...
    \item \textbf{Single-pass efficiency.} The graph Laplacian is computed once from KNN-sparsified similarities and held fixed; the completion is graph-regularized in a single pass, with no alternating refinement or iterative graph recomputation. This avoids the computational overhead and additional tuning parameters of alternating schemes while preserving the graph-regularization mechanism.
\end{enumerate}
...
SGRMC integrates three components into a single-pass framework: KNN graph sparsification, graph Laplacian regularization, and nuclear norm matrix completion with ADMM (Figure~\ref{fig:framework}). We describe each in turn before presenting the full algorithm.

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig2_framework.pdf}
\caption{Overview of the SGRMC framework. Raw similarity graphs are sparsified to their $k$-nearest neighbors per node (density-adaptive), graph Laplacians are computed once, and the association matrix is completed in a single pass (Plan~A: graph-regularized ADMM; Plan~B: standard ADMM followed by an exact Cholesky bilateral filter).}
\label{fig:framework}
\end{figure*}
...
Given a similarity graph $\mathbf{S}$ with degree matrix $\mathbf{D} = \operatorname{diag}(\mathbf{S}\mathbf{1})$, the symmetric normalized Laplacian is $\mathbf{L} = \mathbf{I} - \mathbf{D}^{-1/2} \mathbf{S} \mathbf{D}^{-1/2}$ \cite{chung1997}; its eigenvalues lie in $[0, 2]$, so its spectral norm satisfies $\|\mathbf{L}\|_2 \le 2$. The graph smoothness of a matrix $\mathbf{M}$ with respect to the drug and disease similarity graphs is measured by the Laplacian quadratic form:
...
\begin{equation}
\tilde{\mathbf{M}} = (\mathbf{I} + \alpha \mathbf{L}_{\text{dis}})^{-1}
\, \mathbf{M} \,
(\mathbf{I} + \alpha \mathbf{L}_{\text{drug}})^{-1}
\label{eq:gf}
\end{equation}

Equation~\eqref{eq:gf} describes a bilateral graph low-pass filter: each row $i$ of $\mathbf{M}$ (a disease) is smoothed by taking a weighted average of rows $j$ where $\mathbf{S}_{\text{dis}}(i,j)$ is large, and analogously each column (a drug) is smoothed using $\mathbf{S}_{\text{drug}}$. When embedded in ADMM iterations, this jointly optimizes low-rankness and graph smoothness. When applied as a post-hoc operation, it performs spectral denoising on an already-completed matrix.
...
where $\mathbf{L}_{\text{dis}}$ and $\mathbf{L}_{\text{drug}}$ are computed once from KNN-sparsified similarity matrices (\eqref{eq:knn}) and remain fixed. The first two terms compose the standard nuclear norm completion objective; the third term, controlled by the filter strength $\alpha$, is the graph smoothness regularizer.
...
For small to moderate matrices, we embed the graph Laplacian regularization directly in each ADMM W-update. Let $\mathbf{W}^{\text{std}}$ denote the standard (unregularized) BNNR W-update---the closed-form minimizer of the data-fidelity and consensus terms before any graph correction---and let $\mathbf{M} = \mathbf{W}_{\text{off}}$ be its off-diagonal drug--disease block, the variable on which the graph terms in Eq.~\eqref{eq:sgrmc_obj} act. The graph smoothness term contributes the gradient $2\alpha(\mathbf{L}_{\text{dis}}\mathbf{M} + \mathbf{M}\mathbf{L}_{\text{drug}})$ on this block. Balancing this gradient against the ADMM consensus penalty $\tfrac{\beta}{2}\|\mathbf{W} - \mathbf{W}^{\text{std}}\|_F^2$ gives the stationary condition $\beta(\mathbf{W}_{\text{off}} - \mathbf{W}_{\text{off}}^{\text{std}}) + 2\alpha(\mathbf{L}_{\text{dis}}\mathbf{W}_{\text{off}} + \mathbf{W}_{\text{off}}\mathbf{L}_{\text{drug}}) = 0$ on the off-diagonal block; taking one explicit step of the corresponding fixed-point iteration yields a bilateral low-pass correction with per-iteration coefficient $2\alpha/\beta$, where the factor $1/\beta$ arises from the ADMM penalty scaling. In practice we damp this coefficient by an empirical factor $1/20$, giving the first-order Neumann approximation:
...
With $\alpha = 0.7$ and $\beta = 10$, the damped per-iteration strength $\alpha/(10\beta) = 0.007$ keeps the scaled filter operator contractive, so the first-order Neumann expansion (\eqref{eq:neumann}) remains accurate ($\|\tfrac{\alpha}{10\beta}\mathbf{L}\|_2 \le 0.014$ for any normalized Laplacian, since $\|\mathbf{L}\|_2 \le 2$), whereas the undamped $2\alpha/\beta = 0.14$ would strain the first-order expansion and destabilize the coupling between the X- and W-updates. The W-update becomes:
...
where $\mathbf{W}_{\text{off}} = \mathbf{M}$ is the off-diagonal drug--disease block of the standard ADMM W-update. This joint optimization produces a solution that is simultaneously low-rank, data-consistent, and graph-smooth.
...
\begin{equation}
\tilde{\mathbf{M}} = (\mathbf{I} + \alpha \mathbf{L}_{\text{dis}})^{-1} \cdot \mathbf{M}_{\text{completed}} \cdot (\mathbf{I} + \alpha \mathbf{L}_{\text{drug}})^{-1}
\label{eq:cholesky}
\end{equation}
...
Algorithm~\ref{alg:sgrmc} summarizes the SGRMC procedure. The box constraint $a \le X_{ij} \le b$ of Eq.~\eqref{eq:mc} is enforced by clipping the completed association block to $[0,1]$ after each W-update (Plan~A) and after the Cholesky filter (Plan~B), so that predicted association scores remain valid probabilities.
...
\begin{tabular}{@{\stepcounter{algline}\thealgline.\quad}p{0.50\linewidth}@{\quad}p{0.22\linewidth}@{}}
...
$\rho \gets |\Omega| / (n_d \cdot n_s)$ & // association density \\
$k \gets 5$ if $\rho > 10^{-3}$ else $50$ & // adaptive KNN sparsification \\
$\mathbf{S}_{\text{drug}} \gets \text{KNN-sparsify}(\mathbf{W}_{rr}, k)$ & \\
$\mathbf{S}_{\text{dis}} \gets \text{KNN-sparsify}(\mathbf{W}_{dd}, k)$ & \\
$\mathbf{L}_{\text{drug}} \gets \mathcal{L}(\mathbf{S}_{\text{drug}})$, $\mathbf{L}_{\text{dis}} \gets \mathcal{L}(\mathbf{S}_{\text{dis}})$ & // graph Laplacians \\
Construct augmented matrix $\mathbf{T}$ (Eq.~\eqref{eq:augmented}) & \\
\textbf{if} $\max(n_d, n_s) \le 1000$ \textbf{then} & \\
\quad $\mathbf{X} \gets \text{graphADMM}(\mathbf{T},\allowbreak \mathbf{L}_{\text{dis}},\allowbreak \mathbf{L}_{\text{drug}},\allowbreak \alpha)$ & // Plan A: embedded Neumann \\
\quad $\mathbf{M} \gets \mathbf{X}[n_d:, :n_d]$ & // extract association block \\
\textbf{else} & \\
\quad $\mathbf{X} \gets \text{ADMM}(\mathbf{T})$ & // standard ADMM + SVT \\
\quad $\mathbf{M} \gets \mathbf{X}[n_d:, :n_d]$ & // extract association block \\
\quad $\mathbf{M} \gets (\mathbf{I} + \alpha\mathbf{L}_{\text{dis}})^{-1}\mathbf{M}$ & // Plan B: row filter \\
\quad $\mathbf{M} \gets \mathbf{M}(\mathbf{I} + \alpha\mathbf{L}_{\text{drug}})^{-1}$ & // Plan B: column filter \\
\textbf{end if} & \\
$\mathbf{M} \gets \operatorname{clip}(\mathbf{M}, a, b)$ & // enforce box constraint $[0,1]$ \\
$\mathbf{M}[\Omega] \gets \mathbf{A}[\Omega]$ & // preserve known entries \\
...
\caption{Main results (10-fold CVc). AUPR for BNNR, SGRMC without KNN sparsification (dense graph, $\alpha=0.7$), and SGRMC with density-adaptive KNN ($\alpha=0.7$) on Fdataset, Cdataset, and DNdataset. Error bars show $\pm 1$ std.}
\label{fig:main}
\end{figure*}

SGRMC substantially outperforms BNNR on all three datasets (Figure~\ref{fig:main}). The AUPR improvements are $+20.6\%$ (Fdataset), $+37.9\%$ (Cdataset), and $+26.6\%$ (DNdataset). AUROC improvements are most pronounced on the ultra-sparse DNdataset ($0.9493$ vs.\ $0.8391$, $+13.1\%$ relative gain), where the Plan~B Cholesky filter suppresses false positives across the full ROC curve; on the denser Fdataset and Cdataset the AUROC gains are modest ($+0.6\%$ and $+0.9\%$), the improvements instead concentrating in precision-oriented metrics (AUPR, P@10, P@20).

The improvement is not uniform across datasets. On Fdataset and Cdataset ($\rho \approx 1\%$), the gains are driven primarily by KNN sparsification (see ablation, Section~\ref{sec:knn_ablation}): sparsifying the similarity graph to $k=5$ neighbors removes noisy edges that distort the Laplacian eigenbasis. On DNdataset ($\rho = 0.015\%$), the picture is more nuanced (Table~\ref{tab:knn}). The graph Laplacian filter is the dominant single contributor---with the dense graph it lifts AUPR from 0.2942 (BNNR) to 0.3661 (Table~\ref{tab:alpha})---but aggressive sparsification ($k=5$) is harmful in this ultra-sparse regime, degrading AUPR to 0.2667. The best configuration therefore combines the filter with \emph{mild} sparsification ($k=50$, AUPR 0.3724). The filter also reduces per-fold variance (AUPR std: $\pm 0.0369$ vs.\ $\pm 0.1162$ for BNNR), pulling outlier predictions toward the neighborhood consensus and providing a floor on per-fold performance.
...
& $k=10$ & $0.9148 \pm 0.0240$ & $0.3215 \pm 0.0884$ & 0.74 & 0.62 \\
...
The optimal sparsification level $k$ depends critically on association matrix density, motivating our adaptive scheme (\eqref{eq:k_adaptive}). This dependence reflects a fundamental trade-off between noise removal and information preservation.
...
We regard single-pass as a deliberate efficiency choice rather than a scientific contribution in its own right. Because the graph Laplacian is computed once from the KNN-sparsified similarities and held fixed, the completion is graph-regularized in a single pass---there is no graph to refine between iterations. Unlike alternating schemes that re-estimate the similarity or graph structure from the current completion during optimization, SGRMC incurs no iterative graph-update overhead, and our ablations (Sections~\ref{sec:knn_ablation} and~\ref{sec:filter_ablation}) show that a single sparsification step already captures the graph structure that drives the performance gains.
...
The two protocols answer different questions. CVa asks: ``can the method separate known associations from a random sample of unlabeled pairs?'' CVc asks: ``can the method rank true associations above all possible candidates for a disease with only partial observed associations?'' The latter is more clinically relevant for drug repositioning, where the practical task is to prioritize a small number of candidates from the entire drug space for experimental validation \cite{gottlieb2011}. We encourage future benchmarking efforts to adopt CVc as a standardized protocol alongside CVa.
...
Second, the Plan~A Neumann approximation (\eqref{eq:neumann}) is accurate only when the scaled filter operator is contractive, i.e.\ $\|(\alpha/(10\beta))\,\mathbf{L}\|_2 \ll 1$. This holds for our parameter settings: since $\|\mathbf{L}\|_2 \le 2$ and $\alpha/(10\beta) = 0.007$, the scaled operator norm satisfies $\|(\alpha/(10\beta))\,\mathbf{L}\|_2 \le 0.014 \ll 1$. Substantially higher filter strengths would require second-order approximations or exact Cholesky solves even for small matrices. The Plan~B Cholesky path does not have this limitation.
...
Under disease-centric cross-validation (CVc), SGRMC achieves AUPR of 0.1686 (Fdataset), 0.1829 (Cdataset), and 0.3724 (DNdataset)---representing relative improvements of $+20.6\%$, $+37.9\%$, and $+26.6\%$ over BNNR. Ablation experiments reveal a density-dependent division of labor: KNN sparsification is the dominant performance driver on moderate-density datasets, where it removes noisy edges that distort the Laplacian eigenbasis; on the ultra-sparse dataset the graph Laplacian filter provides the dominant gain, and is most effective in combination with mild sparsification ($k=50$), which preserves the scarce similarity signal.

Our findings challenge a widely held assumption in graph-regularized matrix completion---that denser similarity graphs provide better regularization. Under realistic, all-unlabeled-as-negative evaluation, the opposite is true on moderate-density datasets: sparsified graphs, by excluding weak and noisy similarity edges, produce a cleaner manifold structure and substantially more accurate predictions. On ultra-sparse data, by contrast, mild sparsification ($k=50$) is required---aggressive sparsification discards the scarce similarity information and degrades performance (Table~\ref{tab:knn}). This principle---sparsify the structural prior before regularizing the completion, with the sparsification level calibrated to data density---may generalize beyond drug repositioning to other matrix completion tasks where side information graphs are available but noisy.
