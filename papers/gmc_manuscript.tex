...
Drug discovery costs exceed \$2.6 billion per approved compound, with development timelines spanning 10--15 years \citep{dimasi2003,paul2010}. Drug repositioning---identifying new indications for already-approved drugs---bypasses early-stage safety trials and substantially reduces both cost and risk \citep{chong2007}. Computational approaches span network-based methods \citep{yamanishi2008,martinez2015,luo2016}, matrix factorization \citep{dai2015,ezzat2017,zheng2013,yang2020msbmf}, graph neural networks \citep{fu2022,gao2023}, and matrix completion \citep{yang2019,luo2018,mongia2022}.

A powerful and recurring idea is to exploit \emph{multiple} drug and disease similarity views simultaneously. The multi-similarity family provides five drug similarities (chemical structure, ATC codes, side effects, drug--drug interactions, target profiles) and two disease similarities (phenotype, disease ontology) for the Fdataset, Cdataset, CTDdataset2023, and Ydataset benchmarks \citep{yang2020msbmf}; methods that jointly exploit these views---MSBMF \citep{yang2020msbmf}, HGIMC \citep{yang2021hgimc}, and most recently multi-similarity geometric matrix factorization (multiGMF) \citep{multiGMFpreprint}---outperform single-similarity predictors.

The evaluation protocol determines which paradigm wins. Under disease-centric cross-validation (CVc), held-out diseases are entirely unobserved and predictions are ranked within the full candidate pool; local label-propagation and graph-filtering methods excel there. Under the random-entry masking protocol (CVa)---where 10\% of positive and negative associations are held out per fold and the task is to recover them from a partially observed matrix---the problem is a \emph{matrix completion} problem \citep{candes2009}, and the best-performing methods are all global low-rank factorizations or completions: DNMFDDA (deep semi-NMF), ITRPCA (incomplete tensor robust PCA), OMC (overlap matrix completion), and BNNR (nuclear-norm completion). We adopt CVa because it is the standard evaluation setting of the multi-similarity benchmark suite, including multiGMF, a state-of-the-art preprint. This random-entry masking setup mirrors the practical setting in which some already-known drug--disease associations are unobserved at prediction time, whereas disease-centric CVc targets fully cold-start novel diseases and is a distinct prediction task.
