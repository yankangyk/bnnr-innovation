...
\abstract{%
\textbf{Motivation:} Drug repositioning identifies new therapeutic indications for approved drugs at lower cost and risk. Under the random-entry masking cross-validation protocol (CVa) widely adopted by multi-similarity drug--disease prediction methods, the task reduces to a \emph{matrix completion} problem in which state-of-the-art models rely on global low-rank factorization. Nevertheless, these methods suffer two critical drawbacks: (i)~pure low-rank completion discards the local neighborhood information that label-propagation methods exploit, and (ii)~the multi-similarity views are fused implicitly within a single objective, allowing view-specific errors to propagate without correction.\par
\textbf{Results:} We propose \textbf{GMC (Graph Multi-view Completion)}, a unified nuclear-norm completion framework that integrates cold-start-restricted KNN initialization with scale-free multi-view rank fusion. GMC performs low-rank completion under two complementary geometries derived from the same similarity inputs: a matrix geometry built on the joint bipartite+similarity block, and a tensor geometry that preserves each individual similarity slice. Evaluated under 10-fold random-entry CVa, GMC attains AUPR of 0.6569, 0.7285, 0.3714, and 0.7404 on Fdataset, Cdataset, CTDdataset2023, and Ydataset, yielding the highest mean AUPR across all four benchmarks. GMC significantly outperforms every published baseline on CTDdataset2023 and Ydataset ($p=0.002$, paired fold-level Wilcoxon signed-rank test); on Fdataset and Cdataset it surpasses the strongest competitor, DNMFDDA, without reaching statistical significance.\par
...
We evaluate using 10-fold random-entry cross-validation (CVa), the protocol of the multi-similarity family: for each fold, 10\% of the positive associations (and an equal number of negatives) are held out, their entries are zeroed, and the task is to recover them. This is a matrix-completion setting. We report the area under the receiver-operating characteristic curve (AUROC), the area under the precision--recall curve (AUPR), and top-$K$ precision at $K = 10, 20$. Given extreme class imbalance (positive rate $\le 1.1\%$ in all datasets), AUPR is the primary metric \cite{saito2015}. All experiments use a fixed seed (12345); folds are identical across methods, so per-fold paired comparisons are valid.
...
The CVa protocol holds out $10\%$ of positive and negative entries. To test how GMC degrades with the degree of missing data (sparsity), Figure~\ref{fig:robust} re-runs the \emph{identical} model under held-out fractions of $5\%$, $20\%$, and $30\%$, keeping the same fold-generation seed (12345) so the $10\%$ point is the reported protocol itself (that point reuses the reported test-fold results); the $5\%$, $20\%$, and $30\%$ points are new masks at the same seed, not the reported folds. At $5\%$ masking AUPR is $0.6703$/$0.7393$/$0.3895$/$0.7612$ on Fdataset/Cdataset/CTDdataset2023/Ydataset, rising to the reported $10\%$ values $0.6569$/$0.7285$/$0.3714$/$0.7404$, then $0.6284$/$0.7015$/$0.3289$/$0.6982$ at $20\%$ and $0.5717$/$0.6563$/$0.3113$/$0.6514$ at $30\%$. As the mask fraction rises from $10\%$ to $30\%$---tripling the fraction of hidden entries---AUPR falls by $0.085$, $0.072$, $0.060$, and $0.089$ on Fdataset, Cdataset, CTDdataset2023, and Ydataset (about $13\%$, $10\%$, $16\%$, and $12\%$ relative), monotonically and without collapse. Even at $30\%$ masking---roughly one in three associations hidden---GMC retains AUPR of $0.57$--$0.65$, still comparable to several published baselines run at the easier $10\%$ protocol: on CTDdataset2023 the $30\%$-mask GMC ($0.311$) exceeds nine of the eleven baseline AUPRs at $10\%$ masking (only ITRPCA $0.329$ and multiGMF $0.318$ are higher), and on Fdataset and Cdataset it still beats the weaker baselines (four of the eleven on each). The degradation is gradual and roughly linear in the mask rate, not a cliff. GMC is therefore robust to data sparsity, not tuned to the exact $10\%$ protocol.
